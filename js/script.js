    ...
    let intervalo;
    const maxDesplazamiento = -(deslizador.scrollWidth - window.innerWidth); // límite hacia la derecha

    function empezarDesplazamiento(direccion) {
        intervalo = setInterval(() => {
        desplazamiento += direccion * 5;

        // Limita para no pasar de los extremos
        if (desplazamiento > 0) desplazamiento = 0;
        if (desplazamiento < maxDesplazamiento) desplazamiento = maxDesplazamiento;

        deslizador.style.transform = `translateX(${desplazamiento}px)`;
        }, 20);
    }
    ...
    document.getElementById("izquierda").addEventListener("mouseenter", () => empezarDesplazamiento(1));
    document.getElementById("derecha").addEventListener("mouseenter", () => empezarDesplazamiento(-1));
    document.getElementById("izquierda").addEventListener("mouseleave", detenerDesplazamiento);
    document.getElementById("derecha").addEventListener("mouseleave", detenerDesplazamiento);
